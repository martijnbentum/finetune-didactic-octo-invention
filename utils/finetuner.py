...
def collect_pretrained_checkpoints(pretrained_model_dir = None):
    if not pretrained_model_dir:
        pretrained_model_dir = locations.dutch_wav2vec2_the_first
    p = Path(pretrained_model_dir)
    dirs = p.glob('*/pytorch_model.bin')
    dirs = [str(d.parent) for d in dirs]
    best_last = [x for x in dirs if 'best' in x or 'last' in x]
    numbered = [x for x in dirs if 'best' not in x and 'last' not in x]
    numbered = sorted(numbered, key = lambda x: int(x.split('_')[-1]))
    if best_last:
        all_checkpoints = best_last + numbered
    else: all_checkpoints = numbered
...
def make_checkpoint_finetune_list(pretrained_model_dir, steps = steps):
    cps = collect_pretrained_checkpoints(pretrained_model_dir)
    output = []
    for step in steps:
        for cp in cps:
            number = cp.split('_')[-1]
...
def finetune_all_pretrained_checkpoints_in_list(pretrained_model_dir, steps):
    finetune_list = make_checkpoint_finetune_list(pretrained_model_dir, steps)
